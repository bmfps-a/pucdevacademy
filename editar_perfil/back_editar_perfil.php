...
if (isset($_SESSION['emailcolaborador'])) {
    ...
    if (isset($_FILES['foto']) && $_FILES['foto']['error'] == 0) {
        $fotoColaborador = file_get_contents($_FILES['foto']['tmp_name']);
        $fotoColaborador = mysqli_real_escape_string($conn, $fotoColaborador);

        $sql = "UPDATE colaborador_puc SET nome='$nome', cpf='$cpf', ra='$ra', email='$email', telefone='$telefone', foto_colaborador='$fotoColaborador' WHERE email='$email_login'";
    } else {
        $sql = "UPDATE colaborador_puc SET nome='$nome', cpf='$cpf', ra='$ra', email='$email', telefone='$telefone' WHERE email='$email_login'";
    }
