...
if (isset($_SESSION['emailaluno'])) {
    ...
    if (isset($_FILES['foto']) && $_FILES['foto']['error'] == 0) {
        $fotoAluno = file_get_contents($_FILES['foto']['tmp_name']);
        $fotoAluno = mysqli_real_escape_string($conn, $fotoAluno);

        $sql = "UPDATE aluno_puc SET nome='$nome', cpf='$cpf', ra='$ra', email='$email', telefone='$telefone', foto_aluno='$fotoAluno' WHERE email='$email_login'";
    } else {
        $sql = "UPDATE aluno_puc SET nome='$nome', cpf='$cpf', ra='$ra', email='$email', telefone='$telefone' WHERE email='$email_login'";
    }
    ...
    if (!empty($novaSenha) && $novaSenha === $confirmarSenha) {
        ...
        $sqlSenha = "SELECT senha FROM aluno_puc WHERE email='$email_login' AND senha='$senhaAntigaCriptografada'";
        ...
        if (mysqli_num_rows($resultSenha) > 0) {
            $sqlUpdateSenha = "UPDATE aluno_puc SET senha='$senhaCriptografada' WHERE email='$email_login'";
    ...
    header("Location: editar_perfil_aluno.php?success=1");
    exit();
} else {
